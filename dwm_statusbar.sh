...
while true; do
    if [ -e /dev/shm/volume_status ]; then
        ...
        sep="|"
        # myload=$(uptime | awk -F 'load average:' '{ print $2 }' | sed 's/,//g' | sed 's/ /+/g')
        mytemp1="T1:$(sed 's/000$/°C/' /sys/class/thermal/thermal_zone0/temp)"
        mytemp2="T2:$(sed 's/000$/°C/' /sys/class/thermal/thermal_zone1/temp)"
        ...
        main_text="$(get_wifi_status) $mytemp1 $mytemp2 $mymem $mydsk $sep $mydate $sep $mybatt"
    fi
